...
### 2026-09-05 补充：双 MCP 与保存工程渲染

以下为后续实施对固定 Job 合同的补充；下文 Phase 0-D 的不可用状态与无工具范围保留为当时阶段记录。实际实现和验证进度以 Agent 实施方案为准。

外部助手分别使用 AI Canvas MCP 调度导演台任务、查询及收集成果，使用独立 Blender MCP 搭建场景和动画。软件不新增通用 MCP 客户端或任意 Python 转发。编辑前须比对两端的 jobId、sceneId、sceneRevision、sceneSha256；保存返回仍走原生固定操作和成果校验。

固定 `sceneSource` 只接受 `director-scene` 与 `saved-blender`：前者继续由可移植 JSON 设置时间线和摄影机标记；后者仅加载当前 Scene 绑定下、上一份已校验 Manifest 引用的 `.blend`，保留其起止帧、有效 FPS、摄影机、镜头切换标记和动画。保存工程没有前置 `.blend` 时失败，不重建替代场景，也不改变 JSON revision 或将 Blender 专有数据冒充 JSON 同步结果。

保存工程单帧渲染省略目标帧时采用保存时当前帧，明确指定的帧由实际工程时间线校验；打开编辑器保留当前帧。视频使用保存工程的完整起止帧，有效 FPS 为 `fps / fps_base`，实际时间线随成果返回。帧号范围为 0–10,000,000，FPS 为 1–240，视频最多 14,400 帧、最长 600 秒且帧步长为 1；继续固定预览分辨率及产物集合。新建摄影机无需沿用 JSON ID，但使用的摄影机必须属于当前受管场景。所有 `.blend` 加载继续禁用内嵌脚本执行，输出仍是新的不可变 artifact。

为兼容开发热更新与旧桌面后端，既有安装发现结果新增 `supportsSavedScene` 能力声明。缺失或非布尔真值表示不支持：默认操作沿用旧合同，不发送新增字段；明确请求保存工程模式则在启动前返回升级要求。只有后端明确支持且节点已有成果时，共享 UI/MCP 入口才默认选择保存工程。此能力协商不新增 Tauri command、权限或可自由传入的路径。

固定资源包更新为 1.3.3，Python 字节数与 SHA-256 在原生内嵌常量和运行包清单中同步；Job/Manifest 的 schema 版本仍为 1，旧请求按导演场景模式解释。已通过双 MCP 完成七镜头简易动画搭建、保存返回、当前帧回传及 14 秒视频导出的真机验收；实际工程采用不同于初始 JSON 的时间线、FPS 和摄影机集合。具体证据及用户停止后未继续检查的边界记录于 Agent 实施方案，不能由脚本 fixture 或启动回执代替实际成果校验。
