...
> 适用版本：Plugin API v1 / v2<br>
> 插件入口：设置 → 插件 → 导入插件文件夹 / 插件市场

本文档说明如何为 AI Canvas 编写用户插件。API v1 提供节点工具；API v2 在保持兼容的基础上增加自定义节点、宿主管理的模型调用和显式授权文件操作。
...
| 字段 | 类型 | 必填 | 规则 |
|---|---|---:|---|
| `apiVersion` | number | 是 | 节点工具可用 `1`；自定义节点必须使用 `2` |
...
1. 公开 GitHub 仓库根目录包含 `manifest.json` 和 `main.js`；
...
- [ ] `apiVersion` 为 `1` 或 `2`，`entry` 为 `main.js`；
- [ ] 插件 ID 稳定且不会随版本变化；
...
- [ ] 未使用任意网络、任意路径、DOM、Tauri、npm 包或异步 API；模型和文件操作只返回受支持 effect；
- [ ] 已测试“更新当前节点”和“创建派生节点”是否符合预期；
- [ ] `name`、`description` 和工具标题能让用户理解插件用途与出现位置。

## 15. 兼容性约定

- 宿主根据 `apiVersion` 判断能否加载插件；当前支持 `1` 和 `2`；
