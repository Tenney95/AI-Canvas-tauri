...
5. Python 插件会先显示来源、完整代码 SHA-256、宿主权限和“当前用户完整本机权限”警告；
...
Python 插件在安装、更新、从停用状态重新启用和切回上一版本时都会显示原生高风险确认。确认表示用户信任这份本机代码，并不表示 AI Canvas 已对其完成安全审计。
...
修改文件后重新导入相同 `id` 的插件，会先暂存新的不可执行快照。安装记录保存成功后，宿主先撤销旧版本租约，再原子激活新摘要，并保留原来的启用或停用状态以及上一批准版本。摘要比较、数据库保存或激活任一步失败，都会回切旧 active 或移除首次注册，并丢弃未提交的 staged 快照。
...
- 卸载会移除原生注册、本地保存的插件 Manifest 和源码；
