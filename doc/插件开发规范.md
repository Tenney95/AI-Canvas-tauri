...
> 最后更新：2026-08-31；插件入口：设置 → 插件 → 导入插件文件夹 / 插件市场

本文档说明如何为 AI Canvas 编写用户插件。API v1 提供节点工具；API v2 增加自定义节点、宿主管理的模型调用和显式授权文件操作，并让节点工具也能读取模型目录、在弹窗中提供模型下拉、通过宿主 effect 请求模型调用；API v3 增加由用户明确授权的可信 Python 运行时。
...
- 向自定义节点与节点工具提供当前可调用模型的安全目录，并由宿主代为调用模型（API v2 起）；
...
| 值 | 出现位置 | 点击行为 |
|---|---|---|
| `node-context-menu` | 节点右键菜单 | 立即执行工具，`input.parameters` 为空对象；若工具声明了必填字段，则改为先打开宿主弹窗 |
| `node-toolbar` | 匹配节点的上方工具栏 | 打开 Manifest 声明的宿主弹窗，用户确认后执行 |

右键入口本意是“一键执行”，拿不到弹窗参数。因此**当工具声明了 `required: true` 的字段时**（例如必选模型），宿主会把右键入口也改成打开同一个弹窗，让用户补齐参数再执行，而不是把空参数直接交给插件。没有必填字段的工具保持一键执行，`input.parameters` 仍为 `{}`，插件需要自行处理缺省值。
...
API v2 起额外支持 `model` 类型，用于让用户在弹窗里选择模型：
...
`model` 字段要求插件声明 `models.read`，并要求 `apiVersion` 为 `2` 或 `3`；`modelCategories` 只能用在 `model` 字段上。模型下拉只列出已配置凭据的模型，未配置的厂商不会出现。
...
- 返回对象必须包含非空的 `data` 对象，或包含一个 `effect` 请求宿主代执行模型/文件能力（API v2 起）；
...
插件 JavaScript 仍是同步函数。需要异步模型或文件能力时返回一个 `effect`；宿主完成后**再次调用同一个函数**，并在 `input.effectResult` 中给出结果。单次节点或工具运行最多执行 4 个 effect。
      ...
      if (!input.effectResult) {
        return {
          effect: {
            ...
            imageUrls: input.parameters.frameUrls
          }
        };
      }
...
| `type` | 权限 | 输入 | 返回值 |
|---|---|---|---|
| `model.generate` | `models.invoke` | `modelId`、`prompt`、可选 `imageUrls`（字符串数组，最多 256 项）与 `parameters` | 文本为 `{text}`；媒体为 `{url}` |
...
**随模型一起提交的参考图**：自定义节点会从真实的图片、视频和音频连线中自动收集参考媒体；节点工具没有连线，只能通过 `imageUrls` 显式提交。`parameters` 中自行拼接的媒体 URL 不会被采用。
...
宿主只把成功的图片、视频或音频模型返回 URL 加入本次运行的媒体来源集合；文本模型结果和文件 effect 内容不会获得该资格。来源集合在当前执行内跨 effect 轮次累积，因此后一轮可以引用前一轮模型生成的媒体，但不会跨节点或跨运行持久化。
...
只声明 `models.read` 而没有 `models.invoke` 时，`model.generate` 会被拒绝，并以 `{ ok: false, error }` 回到 `effectResult`，不会中断工具。使用 `model` 弹窗字段或 `models.invoke` 都要求 `apiVersion` 为 `2` 或 `3`。
...
- [ ] 使用模型能力时声明了 `models.read`（目录与模型下拉）与 `models.invoke`（`model.generate`），且 `apiVersion` 为 `2` 或 `3`；
...
- 节点工具的 `model` 弹窗字段与宿主 effect 是向后兼容扩展：能力由 `models.read` / `models.invoke` 权限门控，未声明这些权限的插件行为完全不变，已安装的旧插件重新校验也不会失败；
