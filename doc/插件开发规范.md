...
`derived` 只由宿主 effect 填充，插件不能自行登记。当前仅允许 JPEG/PNG/WebP，最多 25 个、单个 4 MiB、总计 48 MiB；它们只在内存中存在，关闭、取消、切项目、插件 revision 变化或 invocation 结束后立即撤销。最终 `create-node-set` 提交时才由宿主把被使用的图像保存进项目目录。
