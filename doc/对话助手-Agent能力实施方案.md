...
#### 逐帧拉片镜头工作流增量（2026-09-05）

- [x] 插件主窗口弹窗按视口扩展（上限 1600 × 1100），iframe 根尺寸明确；内容区纵向滚动、胶片条横向滚动互不撑宽。独立插件在窄屏重排为单列，保留底部输出操作。
- [x] 新增 `video.detectShots` / `video.inspectFrame`，复用内部视频编辑器解码；镜头初筛有区间、解码数量、处理时间和镜头数量上限。逐帧使用真实时间戳，区分末帧与开区间出点，不以固定帧率推算。
- [x] analysis 抽帧支持原子替换派生批次，失败恢复旧资源；主动导出通过 `resource.export` / `resource.createText` 保存到当前项目，保存后复核租约与画布，过期文件回收。
- [x] UI 本地视频操作、导出与模型额度分别有界（96 / 12 / 4），总请求上限 192，取消/关闭仍可用。未扩展网络、任意文件读写或 Agent/MCP 权限。
- [x] 独立插件实现固定间隔/指定帧/自动镜头、拆合与边界校正、首中尾代表帧、30 步撤销重做、AI 自报置信度与人工覆盖、联系表及 JSON/CSV 导出。镜头来源、ID、入出点、实际帧时间和人工复核记录同时保存在图片与分镜行。

检查：宿主相关 6 个 Vitest 文件 93 项通过；插件 14 项通过；应用和测试类型检查、改动文件 ESLint、插件构建/integrity 与差异检查通过。模拟宿主页验证了深色宽屏和浅色 390px 窄屏布局：工作区没有横向溢出，胶片能横向浏览，人工覆盖在 AI 重试后保留，复核后收到模拟分镜提交。**尚未运行真实 Tauri 视频解码、真实模型请求及项目落盘联调**，模拟结果不替代这些验收。此次无新增依赖、安全配置或 schema 修改，未提交、推送或发布；宿主与独立插件需配套更新。

补充检查：Vite 生产构建输出到独立计划目录，通过；仍有大 chunk 与混合动态/静态导入告警，未为本次功能改动无关分包。24 个交付文本文件严格 UTF-8 与替换字符扫描通过；模拟自动镜头、拆分、撤销/重做、合并、指定帧流程通过，无页面错误日志。模拟浏览器和服务已关闭。

本地提交前复核：用户追加紧凑样式要求，独立插件外层/卡片 padding 收到 8px，普通按钮、单行输入和下拉采用 28px 小尺寸，保留画面尺寸与滚动。插件构建和 15 项测试通过；此次重跑宿主 6 个定向测试文件 87 项、应用/测试类型检查和改动文件 ESLint 通过。深色宽屏与浅色窄屏模拟尺寸检查通过；真实 Tauri 联调仍待验收。用户随后授权分别提交宿主和独立插件，其他并行任务改动不纳入，不推送或发布。

#### 拉片图片展示比例修复（2026-09-05）

- [x] 节点集保存图像后复用既有 `computeImageNodeDimensions`，补全 `nodeWidth/nodeHeight`；展示尺寸来自保存后的图像，而非仅依赖插件声明的像素元数据，避免竖图落入默认 280 × 158 横框。
- [x] 读取尺寸后重新校验画布与插件租约，过期时拒绝整批写入并回收本次文件；节点按整行最大高度加标题/间距排列，保留分组坐标与单次 Store 提交。
- [x] 分镜表的画面缩略图改为 `object-fit: contain`，横/竖/方图完整显示，可能出现留白，不再裁切主体。

检查：7 个定向文件 106 项测试、应用/测试类型检查、目标 ESLint、Vite 生产构建及差异检查通过；新增用例在修复前复现失败，覆盖横/竖/方图、混合比例跨行排列和读取尺寸期间画布过期。构建仍有既有大 chunk 与混合导入告警。本次没有启动真实 Tauri 抽帧或修改用户图片文件；已有图片节点的保存尺寸不自动迁移，需重新生成或手动调整。独立插件不变，不新增依赖、接口或权限，尚未提交或推送。

#### 拉片生成反馈与端口连线修复（2026-09-05）

- [x] 独立插件在结果区、画面卡片和固定页脚展示加载动画与实际阶段文字；生成期间同步禁用新渲染控件，成功或异常都结束加载，不伪造百分比，支持减少动态效果设置。
- [x] 宿主节点集明确使用 `sourceHandle: right` → `targetHandle: left`，修复图片节点默认取左侧 source handle 的问题。只影响新生成连线，保留已保存画布。
- [x] 宿主运行时 52 项、插件 20 项测试通过，插件构建同步 UI integrity；应用类型、定向 ESLint 通过。模拟界面验证深色 1280×720、浅色 390×700 的加载状态、动态表单禁用和模型失败后的恢复；不代表真实 Tauri/付费模型/节点生成已验收。
- [ ] 用户已单独确认专用 capability 清单：插件窗口仅会话桥接，主窗口创建、关闭和回复管理命令。依赖层窗口隔离检查尚未完成，未开放窗口或桥接。

### 12.2.4 平台补充：插件独立 WebView 的权限前置与规范（2026-09-05）

**状态：** `[in-progress]`；阶段 1、追加 Channel 前置修复及阶段 2 原生窗口层已实现并定向验证；阶段 3 主窗口 Broker、展示声明与受门禁保护的路由已接入，真实窗口验收仍待完成。

用户确认复杂插件采用专用 Tauri WebView 直接承载 UI、不嵌套 iframe，并单独确认将 `src-tauri/build.rs` 与 ACL 回归纳入前置修复。现有主窗口弹窗不变；阶段 1 不新增依赖、不修改 `tauri.conf.json` 或窗口 capability，不增加插件文件、网络和原生命令权限。后续依赖层例外已获单独确认：只维护锁定 Tauri 2.11.2 的本地源补丁与相关测试，不升级版本或引入另一套第三方库。

- [x] 核实锁定版 Tauri 源码及已有生成清单：自定义权限 TOML 已启用应用 ACL，不能根据未调用 `AppManifest::commands` 推断为 ACL 未生效。实际问题为 77 个注册命令中的 `read_plugin_package_resource` 漏列第一方权限；只补齐该项，保留其原生调用方、插件摘要、活动 revision 与包资源校验。
- [x] 构建期对比唯一平铺 `invoke_handler` 注册表与根级 `permissions/*.toml` 的显式 allow/deny 集合；缺项、陈旧命令、重复注册或不支持的声明写法直接失败，不自动授予权限。沿用自定义 TOML，不生成重复的逐命令权限文件。
- [x] 增加 7 项构建规则单元测试；真实仓库用例先复现包资源命令漏项，再通过修复。增加第一方 capability 配置回归，确认 `main`、`asset-search`、`chat-assistant`、`video-editor`、`style-guide` 的原有清单不变，整套应用命令未复制给其他 capability，也未新增通配窗口或远程来源。
- [x] 优化《插件开发规范》：增加扩展方式选择、开发/重导入步骤、UI 摘要计算、文件权限矩阵、UI props 与失败处理、紧凑布局和清理约定、安装/隔离/资源故障排查。明确 `main.js` 同步 QuickJS 与 `ui.js` 异步页面的不同职责；规范完整示例经过实际 v1 解析器与只编译不执行的 JavaScript 语法回归。
- [x] `AGENTS.md` 与方案同步无 iframe 目标：独立来源/存储、最小 capability、专用原生桥接、加载前 CSP/导航限制与会话回收为开放入口的前提；模型、文件 effect、预算和画布写回仍由主窗口负责。不把独立窗口等同于 OS 沙箱或草稿持久化。
- [x] 用户单独确认下一阶段最小权限清单。页面拟由 Rust 编译嵌入专用 HTML/bootstrap，经单独私有协议提供；原 `plugin-ui` 协议仍仅供 main 内嵌展示使用。
- [x] 用户单独确认并完成 Channel 依赖层前置补丁：Cargo 精确锁定原 Tauri 2.11.2，通过本地 vendor 覆盖，不升级版本；队列归属真实 WebView 实例，关闭清空并拒绝迟到发送，错误窗口不消耗原消息，同名新实例不继承旧消息，两条大回包路径统一受控。移除内置 fetch 的 ACL 豁免，旧 wire 名仅在 ACL 查询时映射到 core:channel；第一方保留既有 core:default，插件不获该通用权限。
- [x] 新增 5 项标准库队列与 12 项 MockRuntime 定向回归通过；后者读取实际生成的 ACL，并用真实 default capability 核验 5 个第一方窗口，涵盖大 JSON/二进制、流式序号、跨窗口、重复取数、无权限/远程来源、显式拒绝及关闭失效。默认特性 cargo check 通过。测试 target 使用可选 tauri-channel-tests 特性，无新库；Windows 仅给测试补主程序已有的 Common Controls v6 声明，解决进入测试前的缺入口错误，不改系统 DLL。
- [x] 阶段 2：新增固定 `plugin-window` 私有协议、无 iframe 页面与引导脚本；独立 profile + incognito，不加载主入口/Store。Rust 生成不复用标签，固定导航，禁止新窗口、下载与开发者工具，页面响应先施加严格 CSP；不修改 `tauri.conf.json`，原 `plugin-ui` main-only 协议不变。
- [x] 落地已确认的四条命令与专用最小 capability：插件仅会话桥接；创建、关闭、回复在命令内限定 main。主窗口收件 Channel 由真实创建调用绑定，插件回包为 JSON 字符串，无须文件、Shell、通用事件或 core:channel 权限。
- [x] 原生会话绑定插件/工具、三种摘要、项目、节点与 canvas revision；请求、回包和等待任务恢复前重新核验。注册表停用/换版/卸载在原事务锁序内撤销会话，关闭/超时拒绝待处理请求。私有 profile 从不复用，但没有新增自动磁盘清理，不承诺缓存零残留或草稿恢复。
- [x] 定向验证：默认特性 cargo check、49 项插件原生、7 项路径、7 项构建 ACL、86 项前端插件/页面回归、应用及测试类型检查、定向 ESLint 通过；实际 ACL 的 MockRuntime 另验证越权命令与未知窗口失败关闭。普通可选特性 lib test 再遇 Common Controls v6 缺入口，使用只作用于该测试 harness 的 cargo rustc 链接声明完成验证，不改系统 DLL。Clippy 完成，现有 36 项跨模块警告未清理，新窗口与新增注册表逻辑无警告。
- [x] 阶段 3 代码接入：iframe 与原生 Channel 分别验证来源，共用资源/effect/写回 Broker；原生转发逐字段核对完整绑定并拒绝重放。异步 mint 占用会话配额，订阅项目/节点/插件变化，撤销 grant、AbortController 和迟到结果；提交链每轮检查取消与 derivation guard，自己的提交不误报过期。
- [x] 并发打开只创建一份租约，重复打开复用原生绑定与 Channel 聚焦；创建/聚焦与关闭交错时清理迟到窗口。v1 解析器校验 `dialog.presentation`，拉片声明 window；发布门禁未开放时显示回退原因并保留 sandbox 弹窗。不增加权限或依赖。
- [x] 成功提交后立即释放宿主资源，固定引导脚本实际收到成功才发出既有 close 请求；收尾通道最多 30 秒且拒绝其他操作，避免已写入却因关闭抢先而报提交失败。定向回归覆盖该握手、迟到打开/聚焦、请求重放、全部绑定字段、mint 失效、取消、自己的写回和回包失败。
- [x] 阶段 3 验证：宿主 11 文件 195 项、独立插件 20 项定向检查通过，插件构建保持 UI 摘要不变；默认特性 cargo check 和 `--no-default-features` 下 49 项插件原生测试通过（Python 条件用例不代表本轮已执行真实 Python）。应用/测试类型、目标 ESLint、前端生产构建与脚本语法检查通过；生产构建保留既有大 chunk 与混合导入告警。
- [ ] 真实 WebView2 的 CSP、profile/来源隔离、第一方 IPC、深浅主题/缩放、重复聚焦及正确项目画布回填验收；本轮不启动应用、付费模型或真实节点写入，独立用户入口仍未开放。文本生成入口尚不支持远端取消，但会话作废后拒绝其迟到结果与后续 effect。
- [ ] 真实 Tauri 验收：第一方窗口交互兼容、未知/越权窗口 IPC、来源跳转、会话伪造、插件更新/停用/卸载、关闭/切项目以及迟到结果拒绝。静态 capability 检查和模拟测试不替代这些运行态证据。

已执行检查：构建规则 7 项、插件原生 35 项、路径策略 7 项、前端 4 个定向文件 33 项通过；`cargo check --locked --offline --lib`、`npm run test:typecheck`、改动测试文件 ESLint、`build.rs` 定向 rustfmt 与差异检查通过。默认特性 `cargo test` 在本机 VS 2019/ONNX 预编译库链接阶段报 LNK2019/LNK1120、39 个未解析符号；已按现有开发指南使用 `--no-default-features` 运行同域测试，不修改依赖或系统工具链，也不把默认特性测试记为通过。

构建自动更新了 tracked `src-tauri/gen/schemas/acl-manifests.json`；结构化比较确认仅新增 `read_plugin_package_resource`，其它生成内容不变，未手改生成物，未经另外确认不暂存。本批 12 个交付/计划文件严格 UTF-8、替换/乱码字符与 Markdown 围栏检查通过。没有启动应用、真实模型调用或节点写入，未提交/推送。按项目跨层分步检查点，本阶段交付后等待继续确认。回滚仅撤回本批显式文件差异与对应生成清单；无持久化格式或已生成节点迁移，不触碰其他任务改动。

阶段 2 追加生成差异：构建自动同步 `acl-manifests.json`、`capabilities.json`、`desktop-schema.json` 与 `windows-schema.json`，未手改或暂存。阶段 1 上述生成差异记录仅描述当时检查；本阶段新增最小 capability 及四条已批准命令，不扩充 default 的窗口清单。继续保留其他任务工作区差异，未启动真实应用、调用真实模型、写入画布、提交或推送。阶段 2 交付后按跨层检查点暂停，下一阶段继续确认再接前端入口。

#### 插件目录选择提示居中修复（2026-09-05）

- [x] 桌面端点击插件导入区改用已有 Tauri 原生目录选择器，并把返回路径交给既有 `installFromPaths` 校验、摘要与安装链；不再触发 WebView `webkitdirectory` 的顶部“上传整个文件夹”安全提示。
- [x] 浏览器模式继续使用隐藏的 `webkitdirectory` 输入作为兼容兜底；拖拽安装、插件内容读取和原生授权规则不变。
- [x] 原生选择取消不触发安装，选择异常沿用设置页错误提示。未新增依赖、原生命令、文件权限或 Tauri 安全配置。
- [x] 原生扫描与浏览器拖入统一跳过 `.git`、`.hg`、`.svn` 版本库元数据；修复选择源码仓库时 `allow-stat` 在 `.git/COMMIT_EDITMSG` 失败的问题，同时保留 `.github`、源码、文档、UI 和 Manifest 声明资源。

此项属于一次性交互修复。原生系统目录窗口由操作系统/WebView 宿主管理并相对应用显示，应用 CSS 不再尝试定位不可控的浏览器安全提示。对应 5 项路由/过滤测试（含确保 `.git` 不进入受限 `stat`）、TypeScript 类型检查、目标 ESLint、严格 UTF-8 与差异检查通过；未启动真实 Tauri，因此原生窗口的实际视觉位置和真实源码仓库安装仍需桌面端手工验收。
...
## 助手执行可靠性修复（2026-09-05）

本批范围为审计确认的五项执行缺陷，不包含已有节点修改提示词时同步连线，不新增依赖、数据库 schema、工具权限或 Tauri 安全配置。

- [x] 停止/暂停保护：多轮与单工具入口、模型返回后、审批返回后和工具执行/重试前复核信号及任务状态；终止任务不得继续执行排队写入或被旧审批重新启动。
- [x] 模式降级保护：执行时重新读取有效模式并调用既有 Policy；Plan 拒绝写入、B 中未审批的写入拒绝执行；可信 MCP 自主模式覆盖与必须人工作答的 `user_choice` 保持不变。
- [x] 过期批次保护：前序写入失败或画布版本冲突后，剩余写入全部明确拒绝；只接纳成功画布工具返回时固定的版本，不以回调之后的全局版本无条件重设基线。
- [x] 重复执行修复：区分同一请求重放、恢复时的有效画布检查点和新请求；恢复候选跨只读轮次保留，但不包含当前执行段的新步骤，用户插话后清空。画布已变化或旧记录缺少检查点时不猜测复用；修改提示词后重新生成和相同参数的新生成请求正常执行。
- [x] 自主媒体模型修复：C/MCP 保留 Registry 已解析的可用默认/路由模型，B 保持用户选择与确认；两入口均覆盖模式及审批边界，显式 `@model` 一致性与模型可用性校验保持有效。

自动化验证：新增缺陷用例在修复前先复现失败；最终 6 个核心 Agent 文件 74 项通过，扩展至聊天服务、MCP 控制服务与相关 Agent 组件后，54 个文件 480 项通过。应用/测试类型检查、7 个 TypeScript 目标文件 ESLint、8 个目标文件严格 UTF-8/常见乱码扫描及差异检查通过。

相邻测试限制：另两个 MCP 适配器测试在默认 Vitest 加载时因脚本首行 `#!` 报 `Invalid or unexpected token`；该脚本无本批改动，Node 原生语法检查与 ESM 导入通过。仅在内存测试转换中移除首行后，两文件 5 项均通过，未修改脚本或测试配置，因此不将其计为默认命令通过。

验收边界：多轮入口的媒体回归复用真实工具 schema、参数解析和授权，并替换最终执行调用；单工具入口另用可控工具验证有效模式和审批。未启动真实付费生成或桌面交互，已提交厂商的远端请求能否取消、退费不属于本批保证。本批按确认的 8 文件范围完成，保留并行任务改动，尚未提交或推送。

提交前联合复核（2026-09-05）：用户追加授权将当前 17 个文件全部提交到本地 `master`，包含助手五项修复、插件图片比例、吉祥物动作及样式预览；不推送远端。联合定向回归 68 文件 671 项、应用/测试类型检查、15 个 TS/TSX 文件严格 lint 及 17 文件 UTF-8/常见乱码检查通过。既有两个 MCP 脚本测试的默认加载限制仍按上文单独记录，不计入本次通过数；未新增功能或启动真实模型生成。

## 网页读取与中转站配置优化：阶段 A（2026-09-05）

**状态：阶段 A 已完成自动化验证；阶段 B–D 尚未实施。** 用户确认七项优化的文件级方案后，本阶段只处理保存真实性与已有配置保护，按跨层分步检查点交付后等待继续。

- [x] `saveConfig` 增加可选严格错误通道，普通设置按钮保持兼容行为。区分未加载配置、持久化失败、配置已保存但凭据仅会话可用、目录授权同步失败；同时失败时保留两类提示，原始路径和存储错误不进入 Agent 消息。
- [x] `provider_config_apply` 仅在严格保存成功、目标仍匹配时删除草稿并打开设置。失败保留有效草稿供明确重试，不自动重试写入、不整份回滚 Store；草稿已过期时明确提示重新预览，不虚报仍可重试。同一草稿的并发应用互斥。
- [x] 同 ID 模型只更新草稿明确提供的字段；未提供的名称、说明、模态、上下文窗口、参考图模式等保留。明确提供的协议/能力字段整体替换，不混入旧协议字段；分类只有推断且与现值冲突时要求明确分类后重新预览。
- [x] 预览绑定真实目标连接和无凭据配置基线；审批、应用前和持久化后均复核。预览后创建/修改目标连接、保存期间修改目标模型都会被识别；API Key 更新与无关连接修改不误判为目标冲突，旧归一化结果不覆盖新 Store 状态。
- [x] 按 Base URL 并入时，模型绑定已有连接的真实 ID，保留连接名、凭据引用及可见分类设置（包括明确隐藏的空数组）。未涉及模型与其他连接不删除。
- [x] 复用现有 `buildInputDisplay` 和审批详情区展示逐模型更新/保留字段，不新增 UI、权限或消息通道。预览与成功结果明确区分“已解析、本地校验、已保存、未验证实际调用”，不把保存配置描述为真实接口已验证可用。

验证：修复前基线为 3 文件 62 项通过；新增回归先复现 10 项失败。最终阶段定向 3 文件 84 项、扩展 Store/聊天服务/协议导入与自定义视频契约 69 文件 753 项全部通过；应用与测试类型检查、6 个 TS 文件定向 ESLint、7 个交付文件严格 UTF-8/乱码和差异检查通过。

边界：本阶段仅修改已确认的 3 个源码、3 个测试与本实施记录；保留插件 ACL、插件目录选择、导演台等并行差异。未修改网页/Rust/MCP/模型目录、依赖、数据库或 Tauri 安全配置，未启动桌面程序、访问真实厂商或执行付费生成，没有提交或推送。失败后内存配置可能已经生效，但不能保证全部修改已持久化；有效草稿仍受原 30 分钟寿命限制。后续阶段将处理网页多页/SPA、稳定续读/大目录及 MCP 选择回灌，不能将其视为本阶段已完成。

## 网页读取与中转站配置优化：阶段 B（2026-09-05）

**最新状态：阶段 A、B 已完成代码实现与离线自动化验证；真实 WebView 验收未进行，阶段 C、D 待继续。** 用户确认继续后，本阶段处理多页正文/来源错位及短非空 SPA 漏读，不提前实施长文快照、大模型目录或 MCP 类型化审批。

- [x] 原生动态响应改为有界逐页结果，每页保留实际 URL、标题与截断标记；不再拼接多份 HTML 后套用最后一页 URL。所有页正文共享原 1,000,000 字节预算，UTF-8 边界裁剪，新响应不重复携带一份拼接正文。
- [x] 成功页集合放在交互超时 future 外；后续页错误或 180 秒交互超时保留已完成页，不再用当前末页覆盖之前正文。只有一页都没取到才尝试一次受限提取；重复页面、导航失败、空页、体积限制和页数限制分别报告。
- [x] 最多读取 5 页，达到上限时只检查是否还有下一页，不跳入第 6 页；下一页跨域或检查失败不虚报完成。原有匿名 HTTPS、DNS/IP、同源导航、CSP、只读请求、禁登录/表单/下载、窗口清理边界保持，不新增命令或 ACL。
- [x] 前端逐页解析主内容，按各页 URL 解析相对链接，过滤导航/加载/登录壳页，保留 API 代码块缩进。每个有效页分配正文预算，短页余额重新分给长页，避免总内容没超预算也被无谓裁剪。
- [x] 普通网页工具逐页分配引用；搜索导航页及片段不进入事实引用。返回完整性和静态/动态读取方式，明确区分已提取内容与整站遍历，不再承诺“一次返回完整内容”。旧单页响应兼容；旧版拼接多页响应缺少逐页来源时明确要求更新桌面后端，不猜测来源配对。
- [x] 厂商文档去掉“只有完全空白才渲染”的限制，短非空加载壳页先尝试受限渲染；真实短文章保持静态读取。渲染后仍只有壳页时才尝试公开目录，并明确“未读取到目标文档”，不把模型 ID 清单当作请求参数文档。
- [x] 文档既有 offset 分片只附带与片段相交的来源，标题/URL 对应实际片段页；继续读取提示明确使用原始入口和全局 offset。文档工具消费完整性状态，并移除旧正文片段诊断日志；取消后的公开目录结果不再返回成功。

范围：6 个源码（原生 web、webPageService、providerDocsService、webTools、providerConfigTools、chat 类型）、4 个对应测试及本实施记录。文档工具及测试也在用户已批准的 33 文件总清单内；本轮已说明补齐状态展示和去除正文日志的必要性。保留这些文件中阶段 A 的配置修复，以及工作区其他任务的插件/ACL/导演台差异。

验证：新增前端回归先复现 18 项失败；后续复查另复现并修正预算分配与旧多页响应两个边界。最终网页/文档/授权/配置工具定向 7 文件 113 项、扩展 Store/聊天/网页/声明式协议联合回归 72 文件 799 项全部通过。`npm run typecheck`、`npm run test:typecheck`、9 个 TS 文件定向 ESLint、10 个代码/测试文件严格 UTF-8/常见乱码扫描、原生目标 rustfmt 和差异检查通过。

Rust：`cargo test assistant_web::tests --lib --locked --offline --no-default-features` 15 项、`cargo test provider_docs::tests --lib --locked --offline --no-default-features` 5 项通过；默认特性 `cargo check --lib --locked --offline` 通过。默认特性网页测试实际执行后被本机 VS2019/ONNX 预编译库链接阶段以 LNK2019/LNK1120（39 个未解析符号）阻断，未将该命令记为通过，未修改依赖或工具链绕过。

验收边界：前端 HTML 回归使用明确的 DOM fixture 验证编排、文本与链接绑定；原生测试覆盖真实有界收集器、序列化、拒绝路径和超时 future，不等同于真实 WebView 页面交互验收。未启动桌面程序、访问真实厂商、调用付费模型、打包、提交或推送；无数据库升级、依赖或安全配置改动。文档 offset 仍会重抓页面，稳定快照/中段定位属于阶段 C；原生请求仍是超时和结果返回后的取消检查，不是即时中止 IPC。回滚仅撤销本阶段清单内的新增差异，不能整文件覆盖阶段 A 或并行工作。

## 网页读取与中转站配置优化：阶段 C（2026-09-05）

**最新状态：阶段 A–C 已完成代码实现与离线自动化验证，阶段 D 的单工具/MCP 选择回灌仍待实施；真实桌面交互未验收。** 本阶段经用户确认继续，按既有清单实施稳定续读和大目录选择，不提前改变第四阶段的审批类型路由。

- [x] 网页和厂商文档工具接入同一份短期正文快照，返回不透明 `readSessionId`、`nextCursor`、全局 offset 与段落/标题位置。续读和中段定位不重抓网页；旧句柄过期、被清理或作用域不符时明确拒绝，不能悄悄换成新的正文。
- [x] 正文仅放内存：最多 24 份、每份 1,000,000 字符、总计 4,000,000 字符、10 分钟 TTL；并发首次抓取复用同一请求并受数量限制，最多保留 128 个游标和 48 个段落位置。空白密集内容使用线性扫描，避免不可信文本触发正则回溯；返回片段不暴露可修改的内部对象。
- [x] 快照绑定项目、会话、任务、读取类型与原始 URL；每次命中及异步返回后都复核当前授权/取消状态。普通任务完成、预算暂停、中止及上下文装配失败清理正文与目录；清理后的晚到结果不得恢复旧快照。只有精确匹配同项目 MCP 控制会话的请求允许跨单次任务复用，并保留 TTL 和当前工具授权。
- [x] 厂商文档仍按任务最多 24 个不同入口页面、80,000 个累计返回正文字符限制；同页多个片段不重复占用页数。每次先预留最多 10,000 字符，并发请求不能透支；完成/释放复核 reservation 身份，旧请求不能释放新请求额度。快照过期后允许明确从头重读，但不返还已消耗预算。
- [x] 公开 pricing 数据建立独立模型元数据目录，助手只需向 `provider_models_select` 传 `catalogId`。目录最多 5,000 个候选、16 份目录、总计 20,000 个模型、10 分钟 TTL；仅保存 ID、名称、分类，不保存原始接口响应或说明正文。候选去重、冲突/长度校验、作用域与过期校验保持失败关闭；分类推断不是接口能力事实。
- [x] 大目录通过既有审批请求携带有界候选元数据快照，模型输入不复制列表。审批卡增加名称/ID 搜索、分类筛选、每页 20 项、跨页保留选择、清空选择、批次上限和过期提示；默认不预选，每批最多 16 个。旧显式 `models` 输入仍最多 200 个，单次配置草稿仍最多 16 个，任务预算保持独立。
- [x] 审批前通过 Registry 解析目录 ID，确认后及执行前重新查验目录和所选 ID；空选择、重复/伪造 ID、超过批次上限或目录失效均拒绝。主窗口仍是唯一审批执行入口，独立窗口沿用原有任务快照与 `resolve_agent_approval` 消息，未新增跨窗口权限或数据通道。
- [x] 来源仍按实际正文片段页面提供；目录兜底与站点公告分开来源，不能替代接口文档。无 scope 的原有网页搜索/文档读取调用保持既有裁剪行为；完整正文不进入新增持久化来源元数据、配置或日志。

范围：13 个源码、10 个测试及本实施记录，全部在已确认总清单内；其中新建 2 个内存服务、2 个服务测试及 1 个审批卡测试。`agentRoundExecutor` 仅扩展共用审批准备以接收目录元数据；`agentToolExecution` 的 provider_models 回灌缺陷仍留待阶段 D，不能宣称 MCP 完整接入流程已修好。保留 A/B 及并行插件、原生 ACL、导演台与构建相关差异。

验证：新增快照测试首次因实现文件尚不存在退出 1（0 项收集），实现后通过；随后修正预算语义断言、测试重复注册、lint 约束并补齐集成回归。最终扩展 Store/聊天/网页/协议及审批卡共 75 文件 830 项通过；`npm run typecheck`、`npm run test:typecheck`、23 个 TS/TSX 文件定向 ESLint、严格 UTF-8/常见乱码扫描与目标差异检查通过。

验收限制：页面测试使用明确 DOM fixture；审批卡使用服务端静态渲染、共享筛选/选择逻辑和序列化快照验证。搜索/分页控件复用 `ui-*`、`canvas-*` 并允许窄容器换行，但未启动真实桌面进行窄宽度、明暗主题、键盘操作或主/独立窗口端到端视觉验收。本阶段没有新增 Rust 改动，因此未将前一阶段 Rust 测试冒充本轮重跑结果；原生即时取消仍未实现。未安装依赖、改安全配置/数据库、调用真实厂商或付费模型、打包、提交或推送。回滚仅撤销本阶段新增差异，保留 A/B 和其他任务修改；后续按阶段检查点继续 D 与联合验收。

## 3D 导演台双 MCP 协作：第一阶段（2026-09-05）

**状态：软件侧接口和离线自动化验证已完成；两个真实 MCP 的连接、场景与动画联调尚未验收。** 用户确认由外部助手同时调用 AI Canvas MCP 和 Blender MCP，并批准第一阶段十文件方案后开始实施。

- [x] 新增 `director_get_state`、`director_set_runtime`、`director_open_blender`、`director_render_frame`、`director_render_video`、`director_get_operation`、`director_cancel_operation` 七个工具，统一通过 Registry 注册；仅对当前项目的 MCP 控制会话开放，发现阶段不依赖任务记录，沿用既有 Policy 和本地 schema 校验。
- [x] 导演节点按钮与 MCP 共用内存操作服务：启动时同步占用节点，同一时刻仅允许一个受管 Blender 编辑会话；得到原生任务回执即返回 `operationId`，后台继续等待保存返回、校验与回填。启动请求结束不会中止已受理的编辑任务，后续用任务 ID 查询或取消。
- [x] 状态只返回任务 ID、场景 ID/版本/摘要、进度及成果节点摘要；查询没有文件收集或画布写入副作用。不返回私有目录、完整 Manifest、媒体路径或原始错误正文；未选择 Blender 安装时 MCP 明确返回准备要求，文件选择仅由用户界面发起。
- [x] 每次回写复核项目、节点、instanceId、场景引用、前一成果引用及 canvas derivation guard；切换后再切回也不能恢复旧任务的写回资格。原生 jobId、操作与场景身份不匹配时取消并报错；取消、失败及过期结果不得回填。
- [x] 保存返回继续要求已校验的 `.blend` 与当前摄影机 PNG，进程退出本身不算成功；本地图片/视频沿用固定原生 Job。视频导出保留已有的可播放视频节点、共享成果引用、当前节点位置/分组及一次撤销行为。任务控制器、订阅和操作记录只在运行期间保留，终态记录有数量上限。

范围：5 个新增文件、5 个既有文件，包含共享类型/服务、导演工具注册、节点接入、4 个测试文件及本实施记录；保留原有导演视频导出改动和工作区其他任务差异。软件没有新增通用 MCP 客户端或任意 Blender Python 转发，Rust、固定 Python 资源、依赖、数据库和 Tauri 安全配置均无本阶段修改。

验证：修改前 4 文件 43 项基线通过；最终共享服务、工具、节点、MCP 控制服务及既有 Blender runtime/facade 共 6 文件 92 项通过。`npm run typecheck`、`npm run test:typecheck`、9 个 TS/TSX 文件定向 ESLint、10 个交付文件严格 UTF-8/常见乱码扫描与目标差异检查通过。检查中修正了测试节点缺少必填标签、严格 TypeScript 不支持的构造参数属性，以及新保存场景引用未归一化导致的错误失效判断；修正测试标签后 MCP 控制服务 14 项再次通过。

验收边界：回归使用可控原生 facade 和真实 Registry/Policy/Store 编排，不代表真实 Blender MCP 已接通；未启动 Blender、操作用户场景、更新客户端配置或打包安装版。第二阶段仍需匹配 Blender Lab 扩展的外部 MCP Server，并核对两侧 jobId/场景身份后完成搭建、保存返回和回传实测。第三阶段仍需解决后台导出以 Director Scene 重设帧范围、FPS 和镜头表的问题，当前不能承诺自由修改 Blender 时间线或新增摄影机后完整保留动画导出。没有提交或推送；回滚仅撤销十文件内本阶段差异，不整文件覆盖并行修改。

## 网页读取与中转站配置优化：阶段 D（2026-09-05）

**最新状态：阶段 A–D 的七项优化代码已实现，本阶段定向及联合离线回归通过；全仓测试类型门禁和真实桌面验收尚未通过。** 用户确认继续后，本阶段只收敛两个既有工具执行入口的审批选择处理，不扩大 Policy、MCP 或媒体调用权限。

- [x] 修复单工具/MCP 将 `provider_models` 误当作媒体选模、错误要求 `modelRef` 的问题。多轮入口与单工具入口共用按审批 kind 校验及回填逻辑；厂商选择仅回填用户返回的 `selectedModelIds`，媒体选择仅回填 `modelRef`，厂商审批快照不再混入媒体专属字段。
- [x] 兼容旧显式候选列表与阶段 C 的 `catalogId`；目录输入仍只传不透明 ID 和最终选择，不要求模型复制候选清单。显式列表审批保存独立候选快照，选择必须来自审批展示的候选且符合最多 16 项的批次限制。
- [x] 空选择、伪造/重复 ID、超量、非字符串 ID、混合两类输入字段、无效媒体值及过期目录均在工具执行前明确拒绝。确认后重新通过 Registry 做输入解析、schema 和当前授权校验；等待期间工具定义被替换时拒绝沿用旧审批。
- [x] 用户停止、目录清理、授权撤销及模式降为 Plan 后不能执行旧选择；既有项目、画布版本、执行前 Policy 和取消检查保持有效，不新增写入或付费调用重试。C/MCP 已有自主媒体选模逻辑与显式媒体引用行为保持兼容。
- [x] B/C/MCP 的 `user_choice` 均等待用户，模型或 MCP 参数预填 `selectedIds` 不能跳过审批，也不能覆盖用户真实勾选。真实 MCP 控制服务处理器的批准、拒绝、取消路径已加入测试；客户端伪造审批解决方法会被拒绝，错误活动会话不能代替用户审批，Plan 不产生该写入审批。

范围：2 个源码、3 个既有测试与本实施记录，全部在已确认阶段清单内；共享函数位于已有 `agentRoundExecutor`，单工具继续动态复用该模块。没有新建执行服务、工具、权限、持久化类型或依赖；保留 A–C 和并行插件/原生 ACL/导演台改动。

验证：修改前 3 文件 45 项基线通过，新增用例先复现单工具回填失败及多轮非法选择进入执行的问题。最终 3 文件 80 项审批定向测试通过；Store、聊天、网页、文档、声明式协议、厂商目录、MCP 控制服务和审批卡联合 80 文件 927 项通过。独立 `npm run typecheck`、5 个 TS 文件定向 ESLint、严格 UTF-8/常见乱码扫描和目标差异检查通过。

未通过的仓库级检查（未隐藏或计入通过数）：

- `npm run test:typecheck` 退出 1：并行导演台修改在 `tests/services/mcp/mcpControlService.test.ts:250` 的节点 fixture 缺少必填 `BaseNodeData.label`。只读确认差异属于并行任务，本批未修改该文件或类型配置。
- 扩大至整个 MCP 目录的命令出现 1 个套件加载失败：`mcpImageResult.test.ts` 导入未改动适配器时报 `Invalid or unexpected token`（0 项收集），其余 77 文件 897 项通过。脚本首行保留 `#!`，Node 原生语法检查通过，与前述既有 Vitest 加载限制一致；未修改脚本、依赖或测试配置。最终 80 文件绿色命令明确选择 MCP 控制服务测试，不包括该失败套件，不能据此声称全仓测试通过。

验收边界：MCP 回归在进程内调用真实控制服务处理器、Registry、Policy 和审批等待/解决流程，候选模型为本地 fixture；没有启动原生 stdio/HTTP 服务、真实桌面或付费模型。A–D 仍缺真实网页/WebView、多窗口审批、窄宽度/明暗主题/键盘交互及真实厂商端到端验收；原生即时取消仍不在本批实现范围。本阶段未修改 Rust，因此没有重跑或冒称重跑前阶段 Rust 检查。没有打包、提交或推送；回滚仅撤销本阶段新增差异，不整文件覆盖此前和并行工作。

### A–D 提交前联合复核（2026-09-05）

用户授权将七项优化全部提交到本地 `master`，不推送远端。提交范围为已确认的 17 个源码、15 个测试与本实施记录共 33 文件；共享文档仅暂存本批助手优化段落，插件独立窗口、导演台、原生 ACL、vendor 等并行修改保持未提交。

本轮实际重跑：80 文件 927 项前端联合回归、网页 15 项与厂商文档 5 项 Rust 定向测试（`--no-default-features --locked --offline`）、默认特性 `cargo check --lib --locked --offline`、`npm run typecheck`、`npm run test:typecheck`、31 个 TS/TSX 文件严格 ESLint、目标 rustfmt、UTF-8/常见乱码和差异检查均通过。此前缺 label 的导演台 fixture 与本轮曾阻断测试类型的插件 fixture 已由并行任务自行修正，本任务未修改相关文件；以上结果更新此前测试类型尚未通过的状态。

既有 MCP 脚本默认 Vitest 加载限制仍按上文保留，不在 927 项绿色联合命令中，也不因此称全仓测试全部通过。未进行真实桌面/WebView、双窗口审批、厂商请求或付费生成；这些验收限制不因本地 Git 提交而消失。

## 3D 导演台双 MCP：保存工程模式与打开回归修复（2026-09-05）

**状态：固定保存工程模式及兼容修复代码已完成，定向检查通过；双 MCP 场景与动画搭建、保存返回、当前帧回传和视频导出已完成真机验收，用户确认整套流程已通并要求停止测试、提交到本地 master。** 实施期间用户报告原有“打开 Blender”提示运行环境错误，已优先处理该回归。

- [x] `sceneSource` 为固定枚举，保存工程模式只采用当前 Scene 绑定下已验证的上一份 `.blend`；缺少成果时拒绝。保留保存工程起止帧、有效 FPS、当前帧、活动摄影机、相机切换标记和动画，允许与旧 JSON 不同的摄影机集合；JSON 可移植权威、不可变成果和结果写回防护保持有效。
- [x] 单帧缺省使用保存时当前帧，视频使用保存工程时间线并返回实际起止帧/FPS。保存工程视频限制为最多 14,400 帧、最长 600 秒、1–240 FPS 和帧步长 1；相机须属于当前场景，仍使用固定输出尺寸、文件集合和原生校验。
- [x] 修复新前端默认发送 `sceneSource`、旧原生请求拒绝未知字段导致的打开失败。默认导演场景 IPC 恢复原字段集合；现有安装发现接口新增 `supportsSavedScene` 声明，缺失或非布尔真值不启用新模式。新后端且已有成果时 UI/MCP 默认采用保存工程；旧后端保留原流程，明确请求新模式则返回升级说明。
- [x] 同步固定资源包 1.3.3、Python 43,941 个规范 LF 字节及内嵌 SHA-256；旧请求仍按导演场景模式解释，不新增 Tauri command、权限、依赖或自由脚本/路径参数。更新 ADR 0010 的权威与兼容合同。

验证：6 文件 112 项前端回归、23 项 Python 纯状态测试、40 项 Blender Rust 测试全部通过；`npm run typecheck`、`npm run test:typecheck`、10 文件定向 ESLint、默认特性 `cargo check --lib --locked --offline`、目标 rustfmt、严格 UTF-8 和差异检查通过。Rust 测试使用 `--no-default-features --locked --offline`；首次 39 项通过、1 项失败，定位为资源安装测试仍期待旧包目录，同步到 1.3.3 后 40 项通过。上述为定向验证，不声称全仓测试通过。

真机证据：用户重新开启软件后，AI Canvas MCP 的新 stdio 会话实际发现 127 个工具，其中包括 7 个导演工具。修复后通过 `director_open_blender` 收到 running/jobId，观察到 Blender 5.2.1 窗口正常响应及其本地 MCP 监听；独立 Blender Lab MCP 读取受管场景身份、3 个对象、活动摄影机、1–120 帧/24 FPS 和 1 个窗口。其 jobId、sceneId、sceneRevision、sceneSha256 与 AI Canvas 的活动操作全部一致。开发环境中途自动重编译曾导致旧应用和受管进程退出；最终证据来自重启后稳定的同一会话，不把中途回执当作完成。

后续真机验收：按用户提供的分镜 8–14，通过 AI Canvas MCP 创建独立导演节点“警车出动 · 分镜8–14验收”（显示编号 26），经独立 Blender MCP 搭建 318 个对象、7 台摄影机及 34 个带动画的对象。七个简易镜头覆盖眼神、下令出警、车辆跟拍、道路行驶、踩刹车、推手刹和开门下车；工程改为 1–420 帧、30 FPS，与初始 JSON 的 1–120 帧、24 FPS 不同。已查看七个代表帧并修正关键构图，按用户要求仅作为 AI 视频生成的动作和镜头参考。

- [x] Blender MCP 调用既有保存返回操作得到 `FINISHED`；AI Canvas 操作最终为 `succeeded`，Manifest r1 同时包含当前第 390 帧 PNG 和 `.blend`。
- [x] AI Canvas MCP 使用保存工程模式、省略目标帧导出单帧，Manifest r2 实际返回第 390 帧，确认没有回退旧 JSON 时间线或缺省起始帧。
- [x] 保存工程视频导出最终为 `succeeded`，Manifest r3 保留 1–420 帧、30 FPS，画布成功生成视频节点（显示编号 27）。独立 `ffprobe` 读取实际 MP4：H.264、640×360、30 FPS、420 帧、14.000 秒；视频、工程和 PNG 的副本大小及 SHA-256 与成果清单一致。
...
## 插件独立窗口：受限 MCP 验收入口（2026-09-06）

**状态：受限接口与定向检查已完成；真实应用已恢复 MCP 连接，但运行中的工具列表尚未加载新增接口，原生页面隔离验收仍未完成。** 逐帧拉片保持独立仓库、用户自行安装，不作为内置插件；普通入口的真实验收门禁仍然关闭。

- [x] 增加 `plugin_window_get_state`、`plugin_window_open`、`plugin_window_close` 三个 Registry 工具，查询为 `read`，打开/关闭为 `config_write`；仅供当前项目的 MCP 控制会话使用，执行时重新核验项目和取消状态。
- [x] 打开只接受已安装、启用、节点类型匹配且声明独立 UI 的工具。导出别名从活动 Manifest 读取；JSON 初始参数有深度、项数与 16 KiB 总量限制，不接受源码、URL、原生标签、session binding 或任意 `invoke`。
- [x] 正式构建在原生门禁通过前拒绝打开；第一方开发构建可用同一桥接验收，不让插件通过参数解除门禁。插件 capability、原生命令和既有 effect/画布权限没有扩大。
- [x] 状态仅投影目标、创建/收尾阶段与桥接计数，不含摘要、会话秘密、参数、资源、页面正文或原生异常路径。`open` 和 `contextResponded` 不是界面挂载或浏览器隔离通过的证明。
- [x] 关闭按当前项目登记目标定位，同步撤销租约；区分关闭命令回执与操作系统实际销毁，创建中的迟到窗口继续撤销。新增打开操作可在资源暂存期间取消，取消重复聚焦不会撤销原有窗口。

范围：窗口服务、MCP 界面工具、两份既有服务测试、插件开发规范与本实施记录，共六个文件；保留此前用户安装规则回归，以及其他任务的模型选择器、连接设置和构建等差异。未修改 Rust、Tauri 安全配置、依赖或数据库，不提交、不推送，也未调用模型或改动正式画布。

验证：两份直接回归 55 项通过；扩展至 UI 会话、MCP 控制服务、插件 Store 后五文件共 114 项通过。应用类型、测试类型和四个 TypeScript 文件定向 ESLint 通过。首轮测试类型发现异构 `it.each` 参数推导失败，改为显式案例类型后重跑通过；没有以测试运行成功代替类型检查。

真实状态：复用原有本机 MCP 适配器连接成功，读取临时验收画布及交互状态，无生成请求或阻塞弹窗。成功时仍只发现 127 个旧工具，没有冒用尚不存在的新工具名。最后复查连接被重置，服务只读握手明确返回 `MCP_CLIENT_LIMIT_REACHED`（4 个适配器名额已占满）；未断开其他客户端或提高上限。需先释放闲置连接并重新加载应用，再继续受控原生窗口、CSP、存储、越权 IPC、关闭/切项目与回填验收。前端 mock 回归不计为真实 WebView2 验收，不能据此开放普通入口。回滚仅撤销本批六文件的新增差异，不覆盖用户安装规则或其他任务改动。

### 本地 master 归档（2026-09-06）

用户随后要求先提交到本地 `master`，不推送。本次归档纳入上述六文件和此前的 `tests/store/plugins.test.ts` 用户主动安装规则回归，共七文件；不包含临时验收脚本、规划文件或其他任务改动。提交前再次运行五文件 114 项定向回归、应用/测试类型检查和五个 TypeScript 文件的严格 ESLint，均通过；源码与文档保持 UTF-8。原生实机验收和普通入口门禁状态不因本地归档改变，本次不继续操作实际软件或 MCP 客户端。

## 逐帧拉片：手动切换窗口与原视频预览（2026-09-07）

**状态：源码、前端构建与定向回归完成；插件仓库已同步新 UI 与摘要，运行中的旧宿主尚未更新。** 用户最终要求默认仍用主窗口弹窗，点击关闭按钮旁的独立窗口按钮后自动关闭原弹窗，并在采样方式和分析设置之间显示原视频。

- [x] 节点工具默认创建隔离 iframe 弹窗；声明 `presentation: "window"` 的自定义 UI 显示手动窗口按钮。Tauri 桌面入口启用，Web 下禁用该按钮并保留弹窗。普通点击和 MCP 使用同一窗口服务。
- [x] 创建或聚焦独立窗口成功后撤销内嵌会话并关闭弹窗；失败保留编辑，处理中去重。独立会话由服务管理，不随原弹窗卸载而关闭；现阶段不迁移未保存镜头、模型选择或结果，按钮提示说明此限制。
- [x] 插件顶部采用采样方式、原视频、分析设置三列，980px 以下依次纵向排列。原视频使用 `<video controls>`，不自动播放或转码；点击胶片/逐帧定位时暂停并同步位置，沿用现有深浅主题变量。
- [x] 复用授权 `resource.readRange`，每段 180 KiB，避免 Base64 超过既有 JSON 字符串上限。插件校验回包句柄、偏移与完整长度后生成当前页面 Blob；限制单视频 16 MiB，超限或不能播放时明确提示，抽帧仍可用。卸载暂停视频、撤销 URL，并阻止迟到读取继续请求。
- [x] UI Broker 将分段读取与模型额度分开，最多 96 次且累计请求 16 MiB；失败也计数。总请求 192 次、每次资源授权和租约校验保持有效。没有新增依赖、权限、Rust 改动、Tauri 安全配置或数据库迁移。

验证：主程序六文件 93 项定向回归、七个改动 TS/TSX 文件 ESLint、应用/测试 TypeScript 检查均通过；临时目录 Vite 生产构建通过，只保留既有 chunk 与动静态导入提示。插件暂存副本 `npm run build` 与 24 项测试通过，包含原文件字节重组、截断/错位/超限拒绝及关闭中止；UI integrity 已刷新。暂存副本检查不代表插件安装目录已更新。

后续目录同步：用户反馈重装后仍无视频，核验确认所选源目录仍为旧 UI。此次按五文件清单备份并同步插件目录，实际目录 `npm run build` 与 24 项测试通过；五文件严格 UTF-8 与暂存摘要逐一匹配，UI 摘要为 `sha256-201bfa4cffc116dc0d795ac5956400735fe028fcee4ff7123e0d19c3a41810e0`。该同步只更新插件开发目录，还需宿主重新构建和应用内更新安装快照。

此前实机证据：通过受限 MCP 打开专用窗口，页面显示 24 项基础探针通过，观察到重复聚焦、合成报告写回和拉片缩略图加载。主窗口存储 canary 的存在未单独确认，基础探针不能代替所有存储、生命周期、主题/缩放、平台与第一方窗口集成验收；本轮自动测试同样不替代真实点击或播放验收。

部署阻塞：向用户指定的兄弟插件目录同步源码、文档并构建的自动审批连续两次超时，命令均未执行；MCP 只读交互状态查询也审批超时。本轮未关闭或强杀用户应用。只读进程检查显示运行的仍为此前构建，需完成目录同步、重新构建/加载宿主和正常插件更新后再实机验收。另一次较早的正常退出曾因活动任务计数被自动审查拒绝，其后核实为本轮 MCP 查询/切项目的记录；未据此修改任务状态。

最新阻塞：上述目录同步超时已解决，MCP 也已恢复可读；交互状态 busy=false、activeRequest=false、projectLoadStatus=ready。活动计数仍为查询自身和旧 MCP 切项目记录；查询任务随后已完成。通过主窗口正常关闭按钮退出的操作被自动审批拒绝，原因为没有明确重启授权且可能影响活动任务和项目状态。已请求用户确认保存、重启和安装验证，保持旧应用运行，未采取间接退出或强杀。

回滚：只撤销本批精确差异，保留原插件仓库和既有用户改动。未提交、推送或发布；旧运行程序未被替换，未调用付费模型或修改正式画布数据。

### 本次本地归档

用户随后反馈“可以了”，要求提交代码。本次按宿主九文件、独立插件五文件的明确清单归档到各自本地 master，不推送；暂存副本、运行程序、构建产物和缓存不纳入提交。不再发起关闭或重启操作；用户反馈与此前自动化、实机证据分别记录，不把本地提交表述为新增运行态验收。
