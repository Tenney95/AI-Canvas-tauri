...
用户在排查后授权统一优化。任务类型：同类缺陷修复，保持既有上传、文件授权和插件信任边界。

- [x] 新增无运行时依赖的媒体 URL 判定工具，按协议和真实主机名区分本地素材与 HTTP 网络资源，兼容旧 `isLocalImageUrl` 导出。
- [x] 扩图补齐 asset/file/blob 输入上传；媒体读取/落盘识别本地路径并复用现有授权复制与取消进度，不把 asset URL 交给 Rust HTTP 下载。
- [x] 替换图片编辑、视频截帧、模型/工作流、项目恢复、日志与插件素材引用中的来源判断；HTTP 资源的路径或参数中出现 asset.localhost 不改变来源分类。日志内嵌的本地地址仍单独脱敏。
- [x] 更新文件模块说明，前端类型检查、20 个源码文件定向 ESLint、严格 UTF-8 与差异检查通过；按用户要求未运行测试、实际上传、付费调用和构建。

范围：`utils/mediaUrl.ts`；`uploadService.ts`、`apimartService.ts`、`fileService.ts`、`storageService.ts`、`comfyWorkflowService.ts`；AI 图片/视频工具、参考素材收集、HTTP 日志和 RunningHub/标准图片/工作流 adapter；图片编辑工具、VideoNode；插件的媒体来源分类辅助函数；本计划与文件模块文档。没有依赖、安全配置、原生命令或持久化结构变更。回滚仅撤销本阶段差异。

运行时验收仍待完成：桌面环境中的真实本地素材上传、跨目录复制、裁剪与截帧未实测；静态检查不能代替这些行为验证。
