...
import type {
  ...
  WorkflowApiReferences, WorkflowApiTaskDescriptor,
...
export function workflowApiConnection(config: ApiProviderConfig | undefined): WorkflowApiConnection {
  if (!config?.apiKey?.trim()) throw new Error('请先配置工作流 API 的 Token');
  if ([...config.apiKey.trim()].some((char) => char.charCodeAt(0) < 32 || char.charCodeAt(0) === 127)) throw new Error('工作流 API Token 包含无效字符');
  return { apiKey: config.apiKey.trim(), baseUrl: normalizeWorkflowApiBaseUrl(config.baseUrl) };
}
...
export async function executeWorkflowApi(params: {
  ...
  if (!projectId || store.currentProjectId !== projectId || (!params.nodeId && !context)
    || (params.nodeId && (!node || node.data.workflowId !== workflow.id || node.data.type !== 'ai-video'))
    || workflow.category !== 'ai-video') throw new Error('工作流与当前项目或视频节点不匹配');
  ...
  const connection = workflowApiConnection(store.config.providers[manifest.connectionId]);
  ...
  const descriptor: WorkflowApiTaskDescriptor = {
    ...
    origin: connection.baseUrl, state: 'submit_unknown',
  };
  ...
  try {
    ...
    stage('准备素材');
    const body = await buildWorkflowApiInputs(manifest, params.prompt, params.inputs, params.references, signal);
    if (!fresh()) throw new Error('画布已变化，未提交工作流');
    stage('提交任务');
    savePendingTask({ nodeId: trackingId, projectId, nodeType: 'ai-video', provider: 'workflow-api',
    ...
    stage('保存产物');
    const saved = await saveCloudWorkflowOutputs(outputs, projectId, workflow.name, fresh, 'AutoDL');
    if (!fresh()) throw new Error('画布已变化，工作流结果已保留，请继续查询');
...
/** AutoDL 尚无已核实的取消端点；仅停止本地等待，保留远端任务身份。 */
