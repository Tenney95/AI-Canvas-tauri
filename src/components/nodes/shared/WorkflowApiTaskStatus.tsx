...
export default function WorkflowApiTaskStatus({ task }: { task: PendingTask }) {
  ...
  return <div className="ui-card m-2 flex flex-col gap-2 p-3 text-xs">
    <p>{task.taskId ? t('任务 {id} 已保留，可继续查询或保存。', { id: task.taskId }) : t('提交状态未知，请到 AutoDL 核对并补充任务 ID，避免重复生成。')}</p>
    {!task.taskId && <input className="ui-input w-full" aria-label={t('AutoDL 任务 ID')} value={taskId} onChange={(e) => setTaskId(e.target.value)} />}
    <button type="button" className="ui-btn ui-btn--sm" onClick={() => {
      if (!task.taskId) {
        if (!/^[\w-]{1,128}$/.test(taskId.trim()) || !task.workflowApi) { store.showToast(t('请填写正确的任务 ID'), 'error'); return; }
