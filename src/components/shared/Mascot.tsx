...
/** 生成竖直胶囊（圆角矩形）形状，用作眼睛 */
function makeEyeShape(width: number, height: number): THREE.Shape {
  const w = width / 2;
  const h = height / 2;
...
export default function Mascot() {
  ...
  useEffect(() => {
    ...
    const hemiLight = new THREE.HemisphereLight(0xffffff, 0x202028, 1.05);
    scene.add(hemiLight);
    ...
    const sphereMat = new THREE.MeshStandardMaterial({
      ...
      roughness: 0.62,
      metalness: 0.0,
      emissive: new THREE.Color(0x8a93ff),
    ...
    const eyeGeo = new THREE.ShapeGeometry(makeEyeShape(0.16, 0.34));
    ...
    const look = new THREE.Vector2(0, 0); // 全局视线方向（目标）
    const localPointer = new THREE.Vector2(0, 0); // 画布内 NDC，仅用于悬浮检测
    let hovering = false;
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      look.x = THREE.MathUtils.clamp((e.clientX - cx) / refX, -1, 1);
      look.y = THREE.MathUtils.clamp(-(e.clientY - cy) / refY, -1, 1);
    };
    ...
    const render = () => {
      ...
      eyeGroup.rotation.y = THREE.MathUtils.lerp(
        eyeGroup.rotation.y,
        px * EYE_MAX_ANGLE,
        FOLLOW_LERP,
      );
      eyeGroup.rotation.x = THREE.MathUtils.lerp(
        eyeGroup.rotation.x,
        -py * EYE_MAX_ANGLE,
        FOLLOW_LERP,
      );
      ...
      const s = THREE.MathUtils.lerp(head.scale.x, targetScale, 0.1);
