  ...
  return (
    <AnimatePresence>
      ...
      {resourceItems.map(({ key, label, sub, icon }) => (
        <AnimatedButton key={key} className="menu-row has-desc"
          onClick={async () => {
            ...
            if (key === 'markdown') {
              ...
              addNode({
                ...
                type: 'ai-markdown',
                position: pos,
                data: { label: 'Markdown 文档', type: 'ai-markdown', role: 'source', prompt: '', output: '', status: 'idle', nodeWidth: 280, nodeHeight: 200 },
              } as never);
            }
          }}
        >
