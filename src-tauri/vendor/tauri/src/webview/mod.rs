...
use crate::{
  ...
  ipc::{
    ...
    InvokeError, InvokeMessage, InvokeResolver, InvokeResponseBody, Origin, OwnedInvokeResponder,
    ScopeObject,
  },
  manager::AppManager,
  path::SafePathBuf,
...
pub struct Webview<R: Runtime> {
  ...
  pub(crate) channel_data: crate::ipc::channel::ChannelDataQueue<InvokeResponseBody>,
  ...
  pub fn on_message(self, request: InvokeRequest, responder: Box<OwnedInvokeResponder<R>>) {
    ...
    let (resolved_acl, has_app_acl_manifest) = {
      ...
      let acl = runtime_authority.resolve_access(
        crate::ipc::channel::acl_command(&request.cmd),
    ...
    if (plugin_command.is_some() || has_app_acl_manifest || !is_local) && invoke.acl.is_none() {
      #[cfg(debug_assertions)]
      {
        ...
        let key = if request.cmd == crate::ipc::channel::FETCH_CHANNEL_DATA_COMMAND {
          "channel"
        } else {
          key
        };
