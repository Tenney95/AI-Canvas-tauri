...
// Keep the v2 wire name while applying the core:channel capability normally.
pub(crate) fn acl_command(command: &str) -> &str {
  if command == FETCH_CHANNEL_DATA_COMMAND {
    "plugin:channel|fetch"
  } else {
    command
  }
}

#[path = "channel_queue.rs"]
mod queue;
pub(crate) use queue::ChannelDataQueue;
...
impl JavaScriptChannelId {
  ...
  pub fn channel_on<R: Runtime, TSend>(&self, webview: Webview<R>) -> Channel<TSend> {
    ...
    Channel::new_with_id(
      callback_id,
      Box::new(move |body| {
        webview.channel_data.ensure_open()?;
        ...
        match body {
          ...
          _ => {
            webview.channel_data.deliver(body, |data_id| webview.eval(format!(
...
#[command(root = "crate")]
fn fetch<R: Runtime>(request: Request<'_>, webview: Webview<R>) -> Result<Response, &'static str> {
  ...
  {
    webview.channel_data.take(id).map(Response::new)
  } else {
